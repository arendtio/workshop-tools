...
- **Log pools:** process modules `log-generator` / `log-analyzer` persist SQLite files under `data/log-pools/` (overwrite on same name; survives until server restart; mount that directory in Docker for persistence). Generator uses agent tool `workshop_log_pool_generate`; analyzer uses read-only `workshop_log_sql` on a dropdown-selected pool.
